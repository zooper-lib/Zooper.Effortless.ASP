...
namespace ZEA.Serializations.NewtonsoftJson.Binders;
...
public sealed class MultiTypeSerializationBinder : ISerializationBinder
{
	...
	///     The name of the assembly. This output parameter is set to null as assembly names are
	///     ignored.
	/// </param>
		...
		out string typeName)
	{
		...
		typeName = GetTypeNameWithoutNamespace(serializedType);
	}

	private static string GetTypeNameWithoutNamespace(Type type)
	{
		return type.DeclaringType != null
			// For nested types, combine the outer class name with the inner class name
			? $"{type.DeclaringType.Name}.{type.Name}"
			// For non-nested types, just return the name (without namespace)
			: type.Name;
	}
}